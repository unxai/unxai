<template>
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="backdrop-blur-xl bg-white/10 rounded-xl border border-white/20 p-8 shadow-2xl relative overflow-hidden dream-form mb-8">
        ...
        <div class="flex justify-between items-center mb-8">
          ...
          <div class="flex items-center space-x-4" v-if="!isNewDream">
            <span class="text-white/70">{{ dream.date }}</span>
            <div class="flex space-x-2">
              <span v-for="tag in dream.tags" :key="tag" class="px-3 py-1 text-sm rounded-full bg-white/10 border border-white/20">
                {{ tag }}
              </span>
            </div>
          </div>
        ...
        <div>
          <label for="tags" class="block text-sm font-medium text-white/90">标签</label>
          <input
            type="text"
            id="tags"
            v-model="tagsInput"
    ...
    <!-- 分析结果区域 -->
    <div v-if="!isNewDream && dream.id" class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <!-- 情感分析 -->
      ...
      <div class="md:col-span-2 backdrop-blur-xl bg-white/10 rounded-xl border border-white/20 p-6 shadow-xl">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div
            v-for="relatedDream in dreamAnalysis.relatedDreams"
            :key="relatedDream.id"
            class="p-4 bg-gray-800/50 rounded-lg hover:bg-gray-700/50 transition-all duration-300 cursor-pointer"
            @click="$router.push(`/dreams/${relatedDream.id}`)"
          >
            <div class="flex justify-between items-center mb-2">
              <h3 class="text-lg font-medium text-white/90">{{ relatedDream.title }}</h3>
              <span class="text-sm px-2 py-1 rounded-full bg-blue-500/20 text-blue-300">{{ relatedDream.similarity }}% 相似</span>
            </div>
            <p class="text-sm text-white/70">{{ relatedDream.date }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  setup() {
    ...
    const dreamAnalysis = ref({
      ...
      relatedDreams: [
        ...
        { id: 4, title: '时光倒流', date: '2024-01-12', similarity: 72 }
      ]
    });

    const saveDream = () => {
      // 处理标签
      dream.value.tags = tagsInput.value.split(',').map(tag => tag.trim()).filter(tag => tag);
      
      // 这里添加保存梦境的逻辑
      console.log('保存梦境:', dream.value);
